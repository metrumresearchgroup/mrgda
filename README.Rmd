...
# nmvalidate
A data validation tool designed to identify errors in a derived NONMEM data set. 
`nm_validate` runs a series of pass/fail checks on a data set and `nm_summary` 
...
Within the setup section, `flags` are defined. In order to use `nmvalidate` the
...
Each flag is intended for a specific type of variable. Note that not all variables in a data set may fit into one of these flags.
The flags are defined as such:

- id: subject ID column
- study: study identification column
- primary_keys: variables such as EVID and DVID where you anticipate no duplicate combinations
- time: time column
...
## Documentation
Public documentation of all functions is hosted at [https://metrumresearchgroup.github.io/nmvalidate/](https://metrumresearchgroup.github.io/nmvalidate/)

## Development

`nmvalidate` uses [pkgr](https://github.com/metrumresearchgroup/pkgr) to manage
