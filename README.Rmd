...
* **File Type Selection**: You must specify the file type (e.g., CSV, SAS7BDAT, XPT) via `.file_types`.
...
* **Metadata Creation**: A key feature is the creation of a metadata folder (named after the output file, without the extension, and located in the same directory). This folder contains:
    ...
    * **`last-run-summary.txt`**: If a previous version of the dataset is available (either locally or from SVN, controlled by `.compare_from_svn`), a summary of data and spec changes is written here and printed to the console.
