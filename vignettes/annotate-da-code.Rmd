...
```{r, message=FALSE}
pk_data <- readr::read_csv(
  if (file.exists("../inst/derived/pk.csv")) {
    "../inst/derived/pk.csv"
  } else {
...
```{r}
annotate_da("ID = 1 has outlier in PK data, will remove", {
  pk_data %>% 
...
```{r}
annotate_da("Show the first few records of ID = 2 from the pk_data data.frame", {
  pk_data %>% 
    filter(ID == 2) %>% 
...
```{r}
annotate_da("Show a vector of all studies in the data set", {
  pk_data %>% 
    distinct(STUDYID) %>% 
    pull(STUDYID)
})
...
```{r}
annotate_da("Total number of PK records that are not BLQ and doses", {
