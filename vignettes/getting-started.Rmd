---
title: "Getting Started with nmvalidate"
...
This "Getting Started with nmvalidate" vignette takes the user through some basic scenarios for using `nmvalidate` to verify that a data set has been derived according to its data specification and common data assembly errors have been avoided. 
...
Within the setup section of the `yaml` file, `flags` are defined. In order to use `nmvalidate` the
names of the flags must match those shown below. Adding variables to these flags will inform `nmvalidate` of
...
# Non-unique baseline covariates checks
...
# Missing time-varying covariate checks
...
The value and benefit of using `nmvalidate` is dependent on the amount of information included in the
