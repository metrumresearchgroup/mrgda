...
test_that("build_run_summary_lines omits missing diff sections", {
  data_diffs <- tibble::tibble(
    name = "N IDs Diff",
  ...
  expect_true(any(grepl("^Data changes:$", lines)))
  expect_false(any(grepl("^Spec changes:$", lines)))
})

test_that("build_run_summary_lines returns header only when there are no diffs", {
  lines <- build_run_summary_lines(
  ...
  expect_equal(lines, c(
    "Generated at: 2026-01-02 03:04:05 +0000",
    "Generated by: tester"
  ))
})
