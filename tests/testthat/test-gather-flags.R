...
test_that("gather-flags standard case: Works with standard case [NMV-GAT-001]", {
  gather_results <- gather_flags(nm, nm_spec)
  expect_equal(gather_results$flags$primary_keys, c("EVID", "DVID"))
})

test_that("gather-flags special cases: Creates a new column for every missing flag [NMV-GAT-002]", {
  gather_results <- gather_flags(nm, nm_spec)
  expect_equal(gather_results$data$nmvalidate_tv_cov_cat[1], "tv_cov_cat flag missing")
})

test_that("gather-flags special cases: Fills in missing flags in spec [NMV-GAT-002]", {
  gather_results <- gather_flags(nm, nm_spec)
  expect_equal(gather_results$flags$tv_cov_cat, "nmvalidate_tv_cov_cat")
})

test_that("gather-flags special cases: Generates error if no flags specified [NMV-GAT-002]", {
