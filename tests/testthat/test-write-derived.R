...
test_that("write_derived does not write latest-data-diff.txt when no changes", {
  diffs_path <- paste0(gsub(".csv", "" , .temp_csv, fixed=TRUE), "/latest-data-diff.txt")
...
test_that("write_derived leaves latest-data-diff.txt untouched on unchanged re-run", {
  withr::with_tempdir({
    .csv <- paste0(getwd(), "/pk.csv")
    write_derived(.data = nm, .spec = nm_spec, .file = .csv, .compare_from_svn = FALSE) %>%
      suppressMessages()
    ...
    diffs_path <- file.path(gsub(".csv", "", .csv, fixed = TRUE), "latest-data-diff.txt")
...
test_that("write_derived writes latest-data-diff.txt with expected content when data changes", {
  withr::with_tempdir({
    .csv <- paste0(getwd(), "/pk.csv")
    write_derived(.data = nm, .spec = nm_spec, .file = .csv, .compare_from_svn = FALSE) %>%
      suppressMessages()
...
test_that("write_derived leaves stale latest-data-diff.txt untouched when spec changes but data does not", {
  withr::with_tempdir({
    .csv <- paste0(getwd(), "/pk.csv")
    write_derived(.data = nm, .spec = nm_spec, .file = .csv, .compare_from_svn = FALSE) %>%
      suppressMessages()

    # Plant a stale latest-data-diff.txt (simulates leftover from old version or prior change)
    diffs_path <- file.path(gsub(".csv", "", .csv, fixed = TRUE), "latest-data-diff.txt")
    writeLines(
      c(
